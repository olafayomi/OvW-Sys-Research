...
def start_server(grpc_ip,grpc_port,
    ...
    srv6_explicit_path_pb2_grpc.add_SRv6ExplicitPathServicer_to_server(
        SRv6ExplicitPathHandler(iproute, csv_file), grpc_server)
...
def parse_arguments():
    ...
    parser.add_argument(
        '-c' '--csv-file', dest='csv_file', action='store',
        default=None, help='Path to csv file to store '
        'steering instructions received and applied to the dataplane'
    )
...
if __name__ == "__main__":
    ...
    start_server(grpc_ip, grpc_port, secure, certificate, key, csv_file)
