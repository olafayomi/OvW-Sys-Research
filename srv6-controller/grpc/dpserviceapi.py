        ...
        with status.context(context):
            ...
            for rule in request.rules:
                ...
                if result.returncode == 0:
                    ...
                    failed.append(rule)
                # Do a source port rule to test
                cmdlist = ['/sbin/ip6tables', '-t', 'mangle', '-A', 'PREROUTING', '-i', 'NULL', '-p', 'NULL', '--sport', 'NULL', '-j', 'MARK', '--set-mark', 'NULL']
                cmdlist[6] = rule.intName
                cmdlist[8] = rule.protocol
                cmdlist[10] = str(rule.DPort)
                cmdlist[14] = str(rule.FwmarkNo)
                self.log.debug("--SPORT COMMAND to be applied: %s" %cmdlist)
                result2 = subprocess.run(cmdlist) 
                self.log.debug("Result of applying command: %s" %result2)
                if result2.returncode != 0:
                    self.log.debug("Error code for adding table is %s" %result2.returncode)
