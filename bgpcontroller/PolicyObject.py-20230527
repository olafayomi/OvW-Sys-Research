...
class PolicyObject(Process):
    ...
    def filter_export_routes(self, export_routes):
        ...
        # 2023-05-26 Update filter_export_routes to handle preference from source peer
        for prefix, routes in export_routes.items():
            for route_t in routes:
                ...
                if route_copy:
                    filtered_routes[prefix].append((route_copy, pref))
        # perform aggregation if required
        # TODO: 2023-05-26 Make self._aggregate_routes handle preference been added
