...
class BGPSpeaker(Process):
    ...
    def _process_send_announcement(self, message):
        ...
        if "local-preference" in message["route"]:
            ...
            if not isinstance(locPrefVal, int):
                try:
                    local_pref_int = int(locPrefVal)
                except ValueError as e:
                    self.log.debug("Invalid local-preference in BGP encode message from peer %s, local-preference attribute will not be included in the message" %(message["peer"]))
                except TypeError:
                    self.log.debug("local-preference supplied is None in BGP encode message from peer %s, local-preference attribute will not be included in the message" %(message["peer"]))
                else:
                    localPrefAttr.local_pref = local_pref_int
                    any_attrs.Pack(localPrefAttr)
                    bgpmsg.pattrs.append(any_attrs)
            else:
                localPrefAttr.local_pref = locPrefVal
                any_attrs.Pack(localPrefAttr)
                bgpmsg.pattrs.append(any_attrs)
    ...
    def _destruct(self):
        ...
        self.log.debug("DEBUG SILENT FAILURE: BGPSpeaker process %s at %s is being destroyed!!!!"
