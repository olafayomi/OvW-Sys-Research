                        ...
                        if (perfpipe, select.POLLIN) in poll.poll(500):
                            ...
                            for dstAddr in self.clients:
                                ...
                                if (time.monotonic() - self.client_dst_msm[dstAddr]['time']) >= self.period:
                                    self.log.info("PARMetricsModule Routing Period Cycle for DST:%s is: %s" %(dstAddr, self.period))
...
class Bandwidth(BasePARModule):
    def __init__(self, name, command_queue, prefixes, peer_addrs, period, datadir):
        super(Bandwidth, self).__init__(name, command_queue, flows, peer_addrs, period, datadir)
...
class TrafficModule(BasePARModule):
    def __init__(self, name, command_queue, flows, peer_addrs, period, datadir):
        super(TrafficModule, self).__init__(name, command_queue, flows, peer_addrs, period, datadir)
