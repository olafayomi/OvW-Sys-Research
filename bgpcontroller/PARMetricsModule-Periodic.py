                        ...
                        if (perfpipe, select.POLLIN) in poll.poll(500):
                            ...
                            for dstPerfMsg in msms.dstMsm:
                                ...
                                for exitnode in dstPerfMsg.node:
                                    ...
                                    nodes_m.append((node, nexthop, delay, estDelay))
                                if dstAddr not in self.clients:
                                    ...
                                    for n in nodes_m:
                                        name, nh, d, estD = n
                                        self.client_dst_msm[dstAddr][name]['estDelay'].append(estD)
                                        self.client_dst_msm[dstAddr][name]['delay'].append(d)

                                self.log.info("PARMetricsModule received measurements for DST:%s from nodes: %s" % (dstAddr, nodes_m))
                                pname, pnextop, pdelay, pestDelay = nodes_m[0]
                                self.log.info("PARMetricsModule number of measurements stored for DST:%s for %s is %s" %(dstAddr, pname, len(self.client_dst_msm[dstAddr][pname]['estDelay'])))
                            if (time.monotonic() - self.client_dst_msm['time']) >= 25.0:
                                ## Full algorithm here:
                                ## Get current path
                                for dstAddr in self.clients:
                                    ...
                                    for e_node in self.client_dst_msm[dstAddr].keys():
                                        estDelayLst = self.client_dst_msm[dstAddr][e_node]['estDelay']
                                        delayLst = self.client_dst_msm[dstAddr][e_node]['delay']
                                        self.log.info("PARMetricsModule number of measurements after 25 seconds for %s Est Delay: %s Measured Delay:  %s" % (dstAddr, len(estDelayLst), len(delayLst)))
                                        nh = self.client_dst_msm[dstAddr][e_node]['nexthop']
                                        n_estD = estDelayLst[-1]
                                        delay_l.append(n_estD)
                                        trend = self._evaluate_rtt_trend(delayLst, estDelayLst)
                                        nodes.append((e_node, nh, n_estD, trend))
                                        if (n_estD < (self.thresholds['gaming'] - 0.5)) and trend == "STABLE":
                                            lower_th_node.append((e_node, nh, n_estD))

                                    for e_node in self.client_dst_msm[dstAddr].keys():
                                        estDelayLst = self.client_dst_msm[dstAddr][e_node]['estDelay']
                                        n_delay = estDelayLst[-1]
                                                ...
                                                if len(best_route) == 0:
                                                    ...
                                                    for sorted_d in sorted_delays:
                                                        ...
                                                        for b_node in nodes:
                                                            ...
                                                            if (pct_diff >= 8) and (trend == "UNSTABLE"):
                                                                self.log.info("PARMetricsModule percentage decrease in change between current path via %s and best path via %s for DST:%s is %s and the trend is %s" % (current_node, b_name, dstAddr, pct_diff, trend))
                                                                self.log.info("PARMetricsModule should also update path for DST:%s to a better one here" % dstAddr)
                                                                best_route = self._fetch_route_for_prefix(b_name, b_nh, dstAddr)
                                                                if Prefix(dstAddr) in self.current_routes:
                                                                    self.current_routes[Prefix(dstAddr)] = best_route[Prefix(dstAddr)][0]
                                ...
                                for key, val in self.client_dst_msm.items():
                                    if key != 'time': 
                                        self.log.info("PARMetricsModule resetting Est Delay and Actual Delay for %s" % key)
                                        for exitNode in val.keys():
                                            val[exitNode]['estDelay'] = []
                                            val[exitNode]['delay'] = []
