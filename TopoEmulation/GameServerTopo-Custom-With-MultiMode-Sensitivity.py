...
class TestTopo(IPTopo):
    ...
    def build(self, *args, **kwargs):
        ...
        for i, router in enumerate(transit_routers, 1):
            ...
            link[router].addParams(ip=(f"100::{i}/48",))
        ...
        for as_num, routers in as_routers.items():
            ...
            for key, router in routers.items():
                if key != 'i':
                    bgp_peering(self, router, internal)# AS internal peerings
        ...
        for as_num in self.as_config.keys():
            if f'R1' in as_routers[as_num]:
                peer_name = f'AS{as_num}R1'
                peer = as_routers[as_num]['R1']
                print(f"Print {type(transit_routers[1])}")
                transit_router_name = 'Tp1ASr2'
                tran_router = transit_router_dict[transit_router_name]
                print(f"Print type {type(tran_router)}")

                print(f'Attempting to set local pref on {transit_routers[1]} for peer {peer}')
                print(f"BGP config exists: {transit_routers[1].get_config(BGP) is not None}")

                bgp_config = transit_routers[1].get_config(BGP)
                print(f"BGP neighbors: {getattr(bgp_config, 'neighbors', 'No neighbors attr')}")

                transit_routers[1].get_config(BGP).set_local_pref(200, from_peer=peer,
                                                                  matching=(acl4, acl))
        
            if f'R2' in as_routers[as_num]:
                peer_name = f'AS{as_num}R2'
                peer = as_routers[as_num]['R2']
                transit_routers[2].get_config(BGP).set_local_pref(300, from_peer=peer,
                                                                  matching=(acl4, acl))
...
class PARNet(IPNet):
    ...
    def execute_multimodal_timeline(self, start_time, game_duration=901, mode_duration=150):
        ...
        for path_key, path_info in self._multimodal_timeline.items():
            ...
            #as_num, router_num = self._find_as_router_for_path(target, group_id, path_name)
        ...
        total_duration = game_duration + 225  # 225s = 90s arrival + 120s settle + 15s buffer
        mode_switches = total_duration // mode_duration
        
        print(f"Total experiment duration: {total_duration}s")
        print(f"Expected mode switches: {mode_switches}")
        
        switch_count = 0
        
        while (time.time() - start_time) < total_duration:
            if switch_count > 0:  # Skip first iteration (already configured)
                ...
                for path_key, path_info in self._multimodal_timeline.items():
                    ...
                    # Extract AS and router information from path_info (now includes target)
                    target = path_info['target']
                    group_id = path_info['group_id']
                    path_name = path_info['path_name']
                    
                    # Find corresponding AS and router with target awareness
                    #as_num, router_num = self._find_as_router_for_path(target, group_id, path_name)
                    as_num = path_info['topology_as']
                    router_num = path_info['router_num']
                ...
                # Log completion status but let experiment continue
                ...
                if active_paths == 0:
                    print(f"\n🏁 All multimodal paths have completed their mode sequences after {switch_count} switches")
                    # Continue experiment for full duration regardless
                
            # Sleep for mode duration
            print(f"    Sleeping for {mode_duration} seconds...")
...
if __name__ == '__main__':
    arg_parser = argparse.ArgumentParser(
            description='IPMininet script for Evaluation with Enhanced Multimodal TC Support',
    ...
    arg_parser.add_argument('-b', dest='basedirname',
                            ...
                            type=str,
                            default='Multimodal-RTT-Emulation-Enhanced')
    ...
    # Load RTT data (keeping existing functionality)
    tp2_rtt = []
    tp2_file = '/home/ubuntu/ping-msms-between-as34410-and-ubisoft-49544/rtts/rtts-via-AS1273-prb51217.csv'
    if os.path.exists(tp2_file):
        with open(tp2_file, 'r') as f:
            header = next(f)
            reader = csv.reader(f)
            for row in reader:
                tp2_rtt.append(float(row[1]))
        print(f"Loaded {len(tp2_rtt)} TP2 RTT values")

    tp3_rtt = []
    tp3_file = '/home/ubuntu/ping-msms-between-as34410-and-ubisoft-49544/rtts/rtts-via-AS1299-prb51217.csv'
    if os.path.exists(tp3_file):
        with open(tp3_file, 'r') as f:
            header = next(f)
            reader = csv.reader(f)
            for row in reader:
                tp3_rtt.append(float(row[1]))
        print(f"Loaded {len(tp3_rtt)} TP3 RTT values")
    ...
    try:
        ...
        # Configure delays (including multimodal first modes)
        ...
        time.sleep(260)  # Wait for network convergence
        ...
        # Generate client names and arrival times
        ...
        if 'gCl43_1' in client_names:
            client_names.remove('gCl43_1')
            random.shuffle(client_names)
            client_names.insert(0, 'gCl43_1')
            print(f"📌 Client gCl43_1 set to arrive first for synchronized analysis")
        else:
            random.shuffle(client_names)
            print(f"⚠️ Warning: gCl43_1 not found, using default ordering")
        ...
        def multimodal_thread():
            # Wait until clients are settled (90s + 120s)
            #time.sleep(210)
            #net.execute_multimodal_timeline(start_time, game_duration=901, mode_duration=args.mode_duration)
            arrival_time = 90
            settle_time = 110
        ...
        for idx, host_name in enumerate(client_names):
            ...
            net[host_name].cmd(f"python simpleGameClient.py -n {host_name} -d 901 -c {cl_data_dir}/{args.date}-{str(args.expt_no)}/{host_name}.csv &> /dev/null &")
